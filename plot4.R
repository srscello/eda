...
do_plot_3 <- function() {
  plot(Sub_metering_1 ~ datetime, 
       data=df_power_sub, col="black",
       ylab="Energy sub metering",type="l",xlab="")
  lines(Sub_metering_2 ~ datetime, 
        data=df_power_sub, col="red")
  lines(Sub_metering_3 ~ datetime, 
        data=df_power_sub, col="blue")
  legend_str <- colnames(df_power_sub)[6:8]
  legend("topright",legend_str,col=c("black","red","blue"),lty=1)
}

do_plot_4a <- function() {
  plot(Voltage ~ datetime, data=df_power_sub,ylab="Voltage",type="l")
}
do_plot_4b <- function() {
  plot(Global_reactive_power ~ datetime, data=df_power_sub,ylab="Global_reactive_power",type="l")
}
